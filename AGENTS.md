...
- Sempre ler o AGENTS.md de tempos em tempos e garantir que cumpre as regras.
...
- Sempre utilizar as ferramentas do CLI, skills, sub-agentes, pesquisar online quando necessario, plugins, print de tela, MCP e tudo que estiver a disposicao.
- Nao ser preguicoso e teimoso.
- Economizar tokens com RTK e filtros mais eficientes.
...
- Nada de try/catch vazio, nada de suppress que esconda erro real, nada de "self-healing" silencioso.
...
- Proibido monkey patch (não aplicável nativamente em C++, mas vale para macros de preprocessador abusivas).
...
- `qmllint -I build/dev` nos arquivos QML quando disponivel.
- `cppcheck --enable=all --inconclusive --suppress=missingIncludeSystem` quando disponivel.
- `semgrep --config=p/c --config=p/security-audit` quando disponivel.
- `gitleaks dir . --redact --exit-code 1` quando disponivel.
...
- Nao misturar idiomas: comunicacao em PT-BR. Codigo/comentarios em Ingles com caracteres ASCII.
...
- Nao quebrar usabilidade entre ciclos, cada ciclo é auto-suficiente em testes e somente é fechado com a estabilidade e usabilidade garantidas.
...
- Existem arquivos frequentemente locais/fora de escopo; nao comitar sem confirmacao: `.envrc`, `.python-version`, `config/* secret_key`, `CMakeUserPresets.json`, ajustes locais de shell, etc.
...
- Utilizar ferramentas CLI.
- Rodar sempre: `cmake --build`, `ctest`, `clang-format --dry-run --Werror`, `clang-tidy`, `qmllint -I build/dev`, `cppcheck`, `semgrep`, `gitleaks`, `trufflehog`, `detect-secrets`.
- Verificar scripts de empacotamento com ShellCheck (`*.sh`) e PSScriptAnalyzer (`*.ps1`).
- Verificar casos de regressao, concorrencia, perda de desempenho e locks.
- Incluir novos testes em casos de omissao.
...
- Run other tools to test, syntax checking, or build verification tool.
- Run other classic testing tools (ctest, QtTest, Catch2).
...
- TRIGGER: If any code review auto, code review_manual, or dependency check tools were called during the conversation AND no "CLARIFICATION:" actions are present in any response, ALWAYS generate this summary.
...
3. Run technical validation before push (`cmake --build --preset dev`, focused `ctest`) when working on this repo.
...
Preferencia: CLI > MCP. Timeout padrao: 240s.

| Ferramenta | Notas |
|---|---|
| clang-format | Formatacao C++ conforme `.clang-format` |
| clang-tidy | Analise estatica C++ conforme `.clang-tidy` |
| cppcheck | Analise estatica C/C++ (dead code, buffer overflows, leaks) |
| cbmc | Model checker C/C++ |
| qmllint | Linter QML |
| qmlformat | Formatacao QML |
| semgrep | Analise estatica multi-linguagem (`--config=p/c --config=p/security-audit`) |
| detect-secrets | Deteccao de segredos em codigo |
| gitleaks | Scanner de segredos em git history |
| trufflehog | Scanner de segredos em repos |
| ggshield | GitGuardian CLI - segredos e SAST |
| snyk | Teste seguranca deps (usar com `--insecure`) |
| shellcheck | Linter shell scripts |
| PSScriptAnalyzer | Linter PowerShell scripts |
| rg | ripgrep, busca regex ultra-rapida |
| jq | Processador JSON CLI |
| pre-commit | Git hooks manager |
| vmmap | Memory profiling macOS |
| valgrind | Memory/CPU profiling Linux (memcheck, callgrind) |
| heaptrack | Heap profiler Linux/KDE |
