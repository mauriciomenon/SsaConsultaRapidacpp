...
Currently configured project: `SSA_Consulta_Rapida_Cpp` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/SSA_Consulta_Rapida_Cpp/SETUP.md` to fill in
   `data/SSA_Consulta_Rapida_Cpp/INFO.md`.
...
`--project-id SSA_Consulta_Rapida_Cpp` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/SSA_Consulta_Rapida_Cpp/`.
