# Agent setup for `SSA_Consulta_Rapida_Cpp`

This is a deepsec scanning workspace. Project `SSA_Consulta_Rapida_Cpp` was just registered
(target: `..`). Setup is incomplete — `data/SSA_Consulta_Rapida_Cpp/INFO.md`
...
2. **Fill in `data/SSA_Consulta_Rapida_Cpp/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id SSA_Consulta_Rapida_Cpp
pnpm deepsec process --project-id SSA_Consulta_Rapida_Cpp
```
