...
- `build/<platform>/<arch>/<toolchain>/<preset>` contains only rebuildable
  compiler products for one target.
- `dist/windows/<arch>/releases/<version>-<sha>` contains immutable final
  deliverables.
- `dist/windows/<arch>/.staging/<run-id>` is owned by one package run and is
